...
int main(int argc, char **argv) {
    ...
    int s=HEIGHT/size;
    int localimg[s][WIDTH]; // local image for each rank
    double AVG = 0;
    ...
    for (int k = 0; k < N; k++) {
        ...
        for(int i=start; i<end; i++){
            for(int j=0; j<WIDTH; j++){
                ...
                localimg[i-start][j] = cal_pixel(c); // Store pixel value in local image
            }
        }
        ...
        if(myrank==0){
    		...
    		MPI_Gather(localimg, s*WIDTH, MPI_INT, image, s*WIDTH, MPI_INT, 0, MPI_COMM_WORLD);
    }
    if(myrank == 0){
